	...
	public void attack(Piece target) {
		...
		if (!(target instanceof SideKick)) {
			...
			if (target instanceof Armored && !((Armored) target).isArmorUp()) {
				c.setPiece(new Armored(getOwner(), getGame(), target.getName()));
			}

			if (target instanceof Medic) {
				c.setPiece(new Medic(getOwner(), getGame(), target.getName()));
			}

			if (target instanceof Ranged) {
				c.setPiece(new Ranged(getOwner(), getGame(), target.getName()));
			}

			if (target instanceof Speedster) {
				c.setPiece(new Speedster(getOwner(), getGame(), target.getName()));
			}

			if (target instanceof Super) {
				c.setPiece(new Super(getOwner(), getGame(), target.getName()));
			}

			if (target instanceof Tech) {
				c.setPiece(new Tech(getOwner(), getGame(), target.getName()));
			}
